...
英文简历支持两条路径：上传原简历直接转换，或将网页优化后的简历转换为英文。真实转换由服务端 `/api/resume-english` 完成，需要在 Vercel 配置 `OPENAI_API_KEY`；可通过 `OPENAI_RESUME_MODEL` 覆盖默认模型。密钥只保存在服务端，英文版 Word/PDF 下载与中文版共用现有额度。
