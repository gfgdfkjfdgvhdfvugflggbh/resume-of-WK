...
向晴简历 Web 应用，包含简历解析、优化预览、Firebase 邮箱认证、每日免费额度、订单与兑换码流程。
...
> 当前 Vercel 静态部署已经可以使用 Firebase 邮箱注册/登录，但免费额度、会员、订单和闲鱼兑换码若要跨设备可靠同步，还需要部署持久化服务端（推荐 Firestore + 受信任的服务端函数）。浏览器本地数据不能作为正式付费权益的最终依据。
